...
void sobel (unsigned char* s, unsigned char* t, unsigned height, unsigned width, unsigned channels) { 
    ...
    #pragma omp target teams distribute parallel for collapse(2) \
    	map(to: s[0:height * width * channels]) \
    	map(from: t[0:height * width * channels]) \
    	private(i, v, u, R, G, B, val, adjustX, adjustY, xBound, yBound)
    for (y = 0; y < height; ++y) {
        for (x = 0; x < width; ++x) {
            for (i = 0; i < MASK_N; ++i) {
            ...
            totalR = std::sqrt(totalR) / SCALE;
            totalG = std::sqrt(totalG) / SCALE;
            totalB = std::sqrt(totalB) / SCALE;
